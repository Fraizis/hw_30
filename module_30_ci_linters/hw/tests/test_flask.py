...
def test_create_client(client) -> None:
    client_data = {"name": "name", "surname": "surname",
...
def test_create_parking(client) -> None:
    client_data = {'address': 'address', 'opened': '1', 'count_places': 5, 'count_available_places': 5}
    resp = client.post(
        "/parking",
        json=client_data
    )
...
def test_client_parking_twice(client) -> None:
    ...
    assert f'Клиент с id {client_data['client_id']} уже припаркован на стоянке с id {client_data['parking_id']}' in resp.text
